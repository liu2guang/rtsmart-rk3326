...
int fbdev_glXMakeCurrent(FBDevGLXContext *ctx) {
    ...
    if (ctx->gl_context == NULL) {
        rt_kprintf("info:glx.cpp: start make gl_context");
        ...
        rt_kprintf("debug:glx.cpp: start make zbuffer (%d, %d, %d, %d)\n", xsize, ysize, mode, n_colors);
        ...
        if (zb == NULL) {
            ...
            rt_kprintf("error:glx.cpp: cannot create zbuffer");
            return 0;
        }
        ...
        rt_kprintf("info:glx.cpp: succ create gl_context at %x\n", ctx->gl_context);
...
void fbdev_glXSwapBuffers(FBDevGLXContext *ctx) {
    ...
    extern rt_device_t opengl_lcd; 
    ZB_copyFrameBuffer(ctx->zb, ctx->screen, ctx->line_length);
    // rt_kprintf("ctx->screen = 0x%.8x %d\n", ctx->screen, ctx->line_length); 
    rt_device_control(opengl_lcd, RTGRAPHIC_CTRL_RECT_UPDATE, NULL); 
}
